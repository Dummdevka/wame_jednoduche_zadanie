    ...
    <div class="container-fluid mt--7 fluid-bg">
        <div class="col-md-12">
            <div class="card p-3 fit-content">
                <form action="{{$update ? route('projects.update', $project->id) : route('projects.store')}}" method="post" class="w-100 d-flex flex-column align-items-center">
                    @csrf
                    ...
                    <div class="w-25">
                        ...
                        value="[26,57]"
                            @foreach($users as $user)
                                <option value="{{$user->id}}" @if($update && in_array($user->id, $selected_users)) selected @endif>{{$user->name}}</option>
                            @endforeach
                        </select>
                    </div>
                    <button type="submit"
